...
def process_html_files(input_dir, output_dir):
    """
    Recursively processes all HTML files in the input directory and saves output in the output directory.
    """
    ...
    for root, _, files in os.walk(input_dir):
        for file in files:
            if file.endswith(".html"):
                ...
                try:
                    logging.info(f"Processing HTML file: {input_path}")
...
def write_output(output_path, elements):
    """
    Writes partitioned elements to the output file.
    """
    try:
