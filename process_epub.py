...
def process_epub_files(input_dir, output_dir):
    """Recursively processes EPUB files."""
    ...
    for root, _, files in os.walk(input_dir):
        for file in files:
            if file.endswith(".epub"):
                ...
                try:
                    logging.info(f"Processing EPUB file: {input_path}")

                    elements = partition_epub(filename=input_path, include_metadata=True)
...
if __name__ == "__main__":
    ...
    # Process EPUB files
    process_epub_files(input_dir, output_dir)