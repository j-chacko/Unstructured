...
def process_msg_files(input_dir, output_dir):
    """
    Processes all .msg files in the input directory and saves extracted content to the output directory.
    """
    ...
    for root, _, files in os.walk(input_dir):
        for file in files:
            if file.endswith(".msg"):
                ...
                logging.info(f"Processing MSG file: {input_path}")
                try:
...
def log_failed_file(input_path):
    ...
    """
    failed_log_path = os.path.join(os.getenv("LOG_DIR"), "failed_msg_files.log")
    with open(failed_log_path, "a", encoding="utf-8") as f:
        f.write(f"{input_path}\n")
    logging.warning(f"Logged failed MSG file for manual review: {input_path}")
...
if __name__ == "__main__":
    ...
    # Process MSG files
    process_msg_files(input_dir, output_dir)