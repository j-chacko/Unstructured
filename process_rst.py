...
def process_rst_files(input_dir, output_dir):
    """
    Recursively processes all .rst files in the input directory and saves output in the output directory.
    """
    ...
    for root, _, files in os.walk(input_dir):
        for file in files:
            if file.endswith(".rst"):
                ...
                try:
                    logging.info(f"Processing RST file: {input_path}")

                    # Partition the RST file
...
def log_failed_file(input_path, error_message):
    ...
    """
    failed_log_path = os.path.join(os.getenv("LOG_DIR"), "failed_rst_files.log")
    with open(failed_log_path, "a", encoding="utf-8") as f:
        f.write(f"{input_path}: {error_message}\n")
    logging.warning(f"Logged failed RST file for manual review: {input_path}")
...
if __name__ == "__main__":
    ...
    # Process RST files
    process_rst_files(input_dir, output_dir) 