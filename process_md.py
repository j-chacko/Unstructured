...
def process_md_files(input_dir, output_dir):
    """
    Recursively processes all Markdown (.md) files in the input directory
    ...
    for root, _, files in os.walk(input_dir):
        for file in files:
            if file.lower().endswith(".md"):  # Case-insensitive file extension check
                ...
                logging.info(f"Processing Markdown file: {input_path}")
                try:
                    process_single_md_file(input_path, output_txt_path, output_json_path, output_annotated_txt_path, root, input_dir)
                except Exception as e:
                    logging.error(f"Error processing {input_path}: {e}")


def process_single_md_file(input_path, output_txt_path, output_json_path, output_annotated_txt_path, root, input_dir):
    """
    Processes a single Markdown file, extracts metadata and content, and writes outputs.
    """
    # Process the Markdown file
    elements = partition_md(filename=input_path, include_metadata=True)
    if not elements:
