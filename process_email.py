...
def process_email_files(input_dir, output_dir):
    """
    Recursively processes all .eml files in the input directory and saves output in the output directory.
    """
    ...
    for root, _, files in os.walk(input_dir):
        for file in files:
            if file.endswith(".eml"):
                ...
                try:
                    logging.info(f"Processing email file: {input_path}")

                    elements = partition_email(filename=input_path, include_metadata=True)
...
if __name__ == "__main__":
    ...
    # Process email files
    process_email_files(input_dir, output_dir)